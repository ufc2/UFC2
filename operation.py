...
class operation(object):
    ...
    def __init__(self, type, pos, data, user):
        self.type = type
        self.pos = pos
        self.data = data
        self.user = user

def infer(v0, v1, user):
    ...
    for diff in diffs:
        ...
        if diff[0] == 0:
            ...
            if op is not None and op.type == 'd':
                op = operation('i', op.pos, data, user)
            else:
                op = operation('i', index, data, user)
            sequence.append(op)
        else:
            op = operation('d', index, data, user)
            sequence.append(op)
            index += len(data)
    return sequence
...
def transform_ii(o1, o2):
    if o1.data == o2.data:
        return None
    o1.data = b'[' + o1.user.encode() + b' insert:' + o1.data + b']'
    o2.data = b'[' + o2.user.encode() + b' insert:' + o2.data + b']'
...
def conflict_resolution(v0, v1, user1, v2, user2):
    seq1 = infer(v0, v1, user1)
    seq2 = infer(v0, v2, user2)
